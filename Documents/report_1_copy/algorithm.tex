...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\ElsIf {$id^v = pred^u$ \textbf{and} $id^v = slid^u$}
		...
		\ElsIf {$id^v = pred^u$ \textbf{and} $id^v \neq slid^u$}
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		\If{$nlts^{v}<nlts^{u}~\Or~nlts^{v}=nlts^{u}~\textbf{and}~lid^{v} < lid^{u}$}
		\State $ height[u] := (\tau ^{v} ,oid^{v},r^{v},\delta ^{v}+1,nlts^{v},lid^v,u) $
		\If{$ (\delta ^{v} \quad \textbf{mod} \quad D \neq 0) $}
